...
export default function DownloadsPage() {
  ...
  const handleCreateDownload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data: GrpcApiResponse = await res.json();
      if (data.item) {
        setDownloads((prev) => [data.item!, ...prev]);
        setFilename("");
        setUrl("");
      ...
      console.error("Failed to create download via gRPC:", err);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-8 py-4">
      ...
      {/* gRPC Integration Status Banner */}
      <motion.div
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {grpcMeta.source === "grpc_server" ? (
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-medium" title={grpcMeta.error}>
                <AlertCircle size={14} />
      ...
      {/* New Download Form Modal/Section */}
      {showAddForm && (
        <motion.div
